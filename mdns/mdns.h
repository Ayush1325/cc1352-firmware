...
static inline int mdns_socket_open_ipv4(const struct sockaddr_in *saddr) {
  int sock = (int)zsock_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
  if (sock < 0)
    return -1;
...
static inline int mdns_socket_setup_ipv4(int sock,
  ...
  zsock_setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuseaddr,
                   sizeof(reuseaddr));
  // #ifdef SO_REUSEPORT
  //   zsock_setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, (const char
  //   *)&reuseaddr,
  //                    sizeof(reuseaddr));
  // #endif
  // zsock_setsockopt(sock, IPPROTO_IP, IP_MULTICAST_TTL, (const char *)&ttl,
  //                  sizeof(ttl));
  // zsock_setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP, (const char
  // *)&loopback,
  //                  sizeof(loopback));
  ...
  // if (zsock_setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&req,
  //                      sizeof(req)))
  //   return -1;
  ...
  if (!saddr) {
    ...
    // zsock_setsockopt(sock, IPPROTO_IP, IP_MULTICAST_IF,
    //                  (const char *)&sock_addr.sin_addr,
...
static inline int mdns_socket_open_ipv6(const struct sockaddr_in6 *saddr) {
  ...
  if (mdns_socket_setup_ipv6(sock, saddr)) {
    LOG_ERR("Failed to configure socket");
    mdns_socket_close(sock);
    return -1;
  }
  return sock;
...
static inline int mdns_socket_setup_ipv6(int sock,
  ...
  //              sizeof(hops));
  //   setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (const char
  //   *)&loopback,
  //              sizeof(loopback));
  ...
  // if (zsock_setsockopt(sock, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *)&req,
  //                      sizeof(req)))
  //   return -1;
  ...
  if (!saddr) {
    ...
    // zsock_setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_IF,
    //                  (const char *)&ifindex, sizeof(ifindex));
  }

  // int ret = zsock_bind(sock, (struct sockaddr *)&sock_addr,
...
static inline int mdns_unicast_send(int sock, const void *address,
                                    ...
                                    size_t size) {
  if (zsock_sendto(sock, (const char *)buffer, (mdns_size_t)size, 0,
...
static inline int mdns_multicast_send(int sock, const void *buffer,
  ...
  if (zsock_sendto(sock, (const char *)buffer, (mdns_size_t)size, 0, saddr,
                   saddrlen) < 0)
    return -1;
  return 0;
}
...
static inline size_t mdns_discovery_recv(int sock, void *buffer,
  ...
  memset(&addr, 0, sizeof(addr));
  mdns_ssize_t ret = zsock_recvfrom(sock, (char *)buffer, (mdns_size_t)capacity,
                                    0, saddr, &addrlen);
  if (ret <= 0)
    return 0;
...
static inline int send_query(const char *query) {
  struct mdns_query_t mdns_query = {
      .name = query, .type = MDNS_RECORDTYPE_PTR, .length = strlen(query)};
  struct mdns_query_t mdns_queries[1];
  mdns_queries[0] = mdns_query;
  return send_mdns_query(mdns_queries, 1);
}
